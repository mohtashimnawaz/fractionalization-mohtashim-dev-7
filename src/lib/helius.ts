...
// Use `HELIUS_API_KEY` in server environment (Vercel/production). Do NOT expose it as NEXT_PUBLIC_*
const HELIUS_API_KEY = process.env.HELIUS_API_KEY || '';
const HELIUS_RPC_URL = `https://devnet.helius-rpc.com/?api-key=${HELIUS_API_KEY}`;

// Avoid printing secrets in browser bundles. If running server-side, we can log that the key is configured.
if (typeof window === 'undefined') {
  if (!HELIUS_API_KEY) {
    console.warn('⚠️ HELIUS_API_KEY not set. Add it to your deployment environment variables.');
  } else {
    console.log('✅ Helius API key configured (server)');
  }
}
...
async function callDASApi<T>(method: string, params: unknown): Promise<T> {
  if (!HELIUS_API_KEY) {
    throw new Error('Helius API key not configured. Please add HELIUS_API_KEY to your environment');
  }
